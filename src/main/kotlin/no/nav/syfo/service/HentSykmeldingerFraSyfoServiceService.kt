    ...
    fun run(): Int {
        ...
        for (sykmelding in result.rows) {
            if (sykmelding.containsKey("HAR_FRAVAER") && sykmelding["HAR_FRAVAER"] == "1") {
                periodeCounter++
                val fravaerResult =
                    databaseOracle.hentFravaerForSykmelding(sykmelding["sm_sporsmal_id".toUpperCase()] as Int)
        ...
        log.info(
            "Antall sykmeldinger som er hentet i dette forsoket:  {} totalt {}, time used {}, lastIndex {}, antall perioder hentet {}",
            result.rows.size,
            counter,
            timeUsed,
            lastIndex,
